...
from pyope.operators import DerivativeOperator, NormalOrderedOperator, BasisOperator
...
EXPORT_PATH = THIS_DIR / "weight8_candidates_for_opedefs.json"
WEIGHT = Fraction(8)
...
def ff_charge(expr):
    ...
    if isinstance(expr, expdesc.RealizedGenerator):
...
def main():
    ...
    candidates = [
        op
        for op in basis.list(weight=WEIGHT)
    ...
    print(json.dumps({"candidate_count": len(candidates)}, ensure_ascii=True))
