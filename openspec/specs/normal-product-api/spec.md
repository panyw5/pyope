# normal-product-api Specification

## Purpose

This specification defines the `normal_product()` helper function, which provides a convenient syntax for computing nested normal ordered products of multiple vertex operators in the pyope library.

### Context

In vertex operator algebra (VOA) computations, researchers frequently need to compute normal ordered products of multiple operators. The existing `NO(A, B)` function only handles two operators, requiring manual nesting for more complex expressions:

```python
NO(A, NO(B, NO(C, D)))  # Manual nesting - verbose and error-prone
```

The `normal_product()` function simplifies this by accepting variable arguments and automatically constructing the right-associative nested structure:

```python
normal_product(A, B, C, D)  # Equivalent to above - concise and clear
```

### Use Cases

- **W-algebras**: Constructing higher-order generators from multiple operators
- **Sugawara construction**: Building stress tensors from current operators
- **Composite operator calculations**: Any scenario requiring multi-operator normal ordering
