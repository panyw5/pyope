...
**测试**：
- 运行 `tests/test_thielemans_eqs.py`
- 创建对应的 Mathematica 脚本进行对比
...
**实现内容**：
- [ ] 为所有算符类实现 `.latex()` 方法
- [ ] 为 `OPEData` 实现 LaTeX 渲染
- [ ] 实现 `_repr_latex_()` 方法用于 Jupyter 自动渲染
- [ ] 实现 `.render()` 方法显示渲染结果

**测试**：
- 在 `demo/latex_display_example.ipynb` 中测试渲染效果
- 验证各种复杂表达式的渲染

**参考资料**：
- 之前删除的 `LATEX_DISPLAY_GUIDE.md`（可从 git 历史恢复）
