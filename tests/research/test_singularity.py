...
def test_singular_vector_analyzer_detects_primary_current_new_module():
    T = BasicOperator("T_sing_primary_new", conformal_weight=2)
    J = BasicOperator("J_sing_primary_new", conformal_weight=1)
    Bosonic(T, J)
...
def test_singular_vector_analyzer_finds_primary_ansatz_solution_new_module():
    T = BasicOperator("T_sing_find_new", conformal_weight=2)
    J = BasicOperator("J_sing_find_new", conformal_weight=1)
    Bosonic(T, J)
