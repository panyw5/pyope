...
def test_singular_vector_analyzer_detects_primary_current():
    T = BasicOperator("T_sing_primary", conformal_weight=2)
    J = BasicOperator("J_sing_primary", conformal_weight=1)
    Bosonic(T, J)
...
def test_singular_vector_analyzer_detects_nonsingular_stress_tensor_descendant():
    T = BasicOperator("T_sing_desc", conformal_weight=2)
    Bosonic(T)
...
def test_singular_vector_analyzer_finds_primary_ansatz_solution():
    T = BasicOperator("T_sing_find", conformal_weight=2)
    J = BasicOperator("J_sing_find", conformal_weight=1)
    Bosonic(T, J)
