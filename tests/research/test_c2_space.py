...
from pyope import BasicOperator, Bosonic, C2Space, LocalOperatorBasis, NO, Zero, d


def test_c2_space_generators_include_virasoro_weight_five_terms():
    T = BasicOperator("T_c2_gen", conformal_weight=2)
    Bosonic(T)
...
def test_c2_space_basis_is_independent():
    T = BasicOperator("T_c2_basis", conformal_weight=2)
    J = BasicOperator("J_c2_basis", conformal_weight=1)
    Bosonic(T, J)
...
def test_c2_space_contains_known_generator():
    T = BasicOperator("T_c2_contains", conformal_weight=2)
    Bosonic(T)
...
def test_c2_space_rejects_non_c2_element():
    T = BasicOperator("T_c2_reject", conformal_weight=2)
    Bosonic(T)
...
def test_c2_space_exposes_reducer_style_quotient_api():
    T = BasicOperator("T_c2_quotient_api", conformal_weight=2)
    Bosonic(T)
...
def test_c2_space_second_derivative_of_generator_is_in_c2():
    ...
    """
    T = BasicOperator("T_c2_ddT", conformal_weight=2)
    Bosonic(T)
...
def test_c2_space_half_integer_weight_derived_element_is_in_c2():
    ...
    """
    G = BasicOperator("G_c2_half", conformal_weight=Rational(3, 2))
    Bosonic(G)
