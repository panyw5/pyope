...
def test_realized_coordinates_produces_free_field_expression():
    ...
    coordinates = realized_coordinates(d(W), free_field_basis, weight=3)

    assert coordinates == free_field_basis.coordinates(2 * NO(d(J), J), weight=3)


def test_realized_coordinates_uses_wolfram_precanonicalization_when_available(
    monkeypatch,
):
    ...
    assert expand_calls == [d(NO(J, J)), 2 * NO(d(J), J)]
    assert coordinates == free_field_basis.coordinates(2 * NO(d(J), J), weight=3)
